...
namespace SampleELT.Steps
{
    public class SqliteInputStep : StepBase
    {
            ...
            CancellationToken ct)
        {
            ...
            if (executeEachRow && inputData.Count > 0)
            {
                foreach (var row in inputData)
                {
                    ...
                    using var cmd = new SqliteCommand(execSql, conn);
                    if (hasNamedParams)
                ...
                progress.Report($"SQLite Input (行ごと実行): {result.Count}行 読み込み完了");
            }
            else if (!executeEachRow && inputData.Count > 0 && (hasNamedParams || sql.Contains('?')))
            {
                ...
                progress.Report($"SQLite Input (パラメータ実行): {result.Count}行 読み込み完了");
            }
            else
            {
                ...
                progress.Report($"SQLite Input: {result.Count}行 読み込み完了");
            }
        ...
        private static void AddNamedParameters(SqliteParameterCollection p, Dictionary<string, object?> row)
        {
            foreach (var kvp in row)
                p.AddWithValue($"@{kvp.Key}", kvp.Value ?? DBNull.Value);
        }

        private static void AddParameters(SqliteParameterCollection p, List<object?> values)
        {
